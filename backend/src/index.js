...
function renderEmailTemplate({ title, greeting, content, details = [], actionButtons = [], importantNote = null, highlight = null }) {
    const detailRows = details.map(d => `
        <tr>
            <td style="border-bottom: 1px solid #e2e8f0; padding: 14px; color: #64748b; font-size: 14px;">${d.label}</td>
            <td style="border-bottom: 1px solid #e2e8f0; padding: 14px; color: #0f172a; font-size: 14px; font-weight: 600; text-align: right;">${d.value || 'N/A'}</td>
        </tr>
    `).join('');

    const buttons = actionButtons.map(btn => `
        <td style="padding: 10px;">
            <a href="${btn.link}" style="display: inline-block; background: ${btn.color || 'linear-gradient(135deg, #2563eb 0%, #7c3aed 100%)'}; color: #ffffff; text-decoration: none; padding: 14px 32px; border-radius: 10px; font-weight: 600; font-size: 15px; box-shadow: 0 4px 12px rgba(37, 99, 235, 0.2);">${btn.label}</a>
        </td>
    `).join('');
